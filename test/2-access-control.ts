...
describe("Access Control", () => {
  ...
  it("Pausable", async () => {
    ...
    await expect(w3Bucket.connect(Alice).safeMint(Bob.address))
      .to.be.rejectedWith(
        /Pausable: paused/,
  ...
  it("Mint Role", async () => {
    ...
    await expect(w3Bucket.connect(Bob).safeMint(Bob.address))
      .to.be.rejectedWith(
        /AccessControl/,
    ...
    await expect(w3Bucket.grantRole(minterRole, Caro.address))
      .not.to.be.reverted;
    await expect(w3Bucket.connect(Caro).safeMint(Caro.address))
      .not.to.be.reverted;
